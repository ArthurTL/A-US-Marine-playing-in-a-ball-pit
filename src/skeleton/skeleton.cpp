...
namespace cgp
{
	...
	buffer<affine_rt> skeleton_animation_structure::evaluate_local(float t) const
	{
		...
		for(size_t kj=0; kj<N_joint; ++kj)
		{
			...
			skeleton_current[kj] = affine_rt(rotation_transform::lerp(T0.rotation,T1.rotation,alpha),T0.translation*(1-alpha)+T1.translation*alpha); // Change this line
		}
	...
	buffer<affine_rt> skeleton_animation_structure::evaluate_global(buffer<affine_rt> const& local, float t) const
	{
		return skeleton_local_to_global(local, parent_index);
	}
